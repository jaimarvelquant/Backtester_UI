...
export function normalizePortfolio(portfolio) {
    ...
    if (copy.startDate)
        copy.startDate = dateLongToString(copy.startDate);
    if (copy.endDate)
        copy.endDate = dateLongToString(copy.endDate);
    if (Array.isArray(copy.strategiesSetting)) {
        copy.strategiesSetting = copy.strategiesSetting.map((strategy) => {
            const strategyCopy = clone(strategy);
            if (Array.isArray(strategyCopy.legsSettings)) {
                strategyCopy.legsSettings = strategyCopy.legsSettings.map((leg) => {
...
export function normalizeTradingView(tv) {
    const copy = clone(tv);
    if (copy.startdate)
        copy.startdate = dateLongToDisplay(copy.startdate);
    if (copy.enddate)
        copy.enddate = dateLongToDisplay(copy.enddate);
    if (Array.isArray(copy.tvsignals)) {
        copy.tvsignals = copy.tvsignals.map((signal) => {
...
export function normalizePortfolioRunResponse(response) {
    const copy = clone(response);
    if (Array.isArray(copy.transactions)) {
        copy.transactions = copy.transactions.map((item) => {
    ...
    metricCollections.forEach(({ key, footerKey }) => {
        ...
        list.forEach((item) => {
            ...
            if (metric.maxProfitDate)
                metric.maxProfitDate = dateLongToDisplay(metric.maxProfitDate);
            if (metric.minProfitDate)
                metric.minProfitDate = dateLongToDisplay(metric.minProfitDate);
            if (metric.backteststartdate)
                metric.backteststartdate = dateLongToDisplay(metric.backteststartdate);
            if (metric.backtestenddate)
                metric.backtestenddate = dateLongToDisplay(metric.backtestenddate);
